...
const palette: Record<AlertType, { ring: string; glow: string; icon: string; tint: string }> = {
  ...
  tip:     { ring: "border-orange-500/30",  glow: "shadow-orange-500/10",  icon: "text-orange-400",  tint: "from-orange-500/10 to-orange-500/0" },
  ...
  return (
    <div
      className={cn(
        ...
        p.ring, p.glow, p.tint, className
      )}
    >
      <div className={cn("shrink-0 w-10 h-10 rounded-lg flex items-center justify-center bg-background/40 border", p.ring)}>
        <Icon className={cn("w-5 h-5", p.icon)} />
      </div>
      <div className="flex-1 min-w-0">
        <h5 className="font-semibold mb-1.5 mt-0 pb-0 border-0 text-foreground">{title}</h5>
